...
class ApkHandler implements Handler
{
    /** @var Executor */
    private $executor;
    /** @var Dirs */
    ...
    public function __construct(Executor $executor, Filesystem $filesystem, Dirs $dirs, string $curlCmd, string $apkCmd)
    {
    ...
    public function handle(PackageConfig $config): InstallResult
    {
            ...
            foreach ($versionData['assets'] as $asset) {
                ...
                foreach ($matches as $match) {
                    if (!preg_match('/'.$match.'/', $asset['name'])) {
                        echo 'no match: '.$asset['name'].' '.$match.PHP_EOL;
                        $hasMatch = false;
                        break;
                    }
                }
        ...
        $commandResult = ($this->executor)($commandParts);
        $commandParts = [$this->apkCmd, 'add', '--allow-untrusted', $target];
        $commandResult = ($this->executor)($commandParts);
