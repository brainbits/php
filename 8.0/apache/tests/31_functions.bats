...
@test "php_set_memory_limit() sets memory limit" {
  run docker run --rm $TAG bash -c 'source /usr/bin/functions/functions.sh && apache_set_php_memory_limit 50M && grep memory_limit /etc/apache2/conf.d/php8-module.conf'
  echo "status: $status"
  echo "output: $output"
  [ "$status" -eq 0 ]
