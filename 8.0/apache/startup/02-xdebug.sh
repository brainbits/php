...
log_setting "create /etc/php8/conf.d/$1_xdebug.ini | + zend_extension=xdebug"
echo zend_extension=xdebug > /etc/php8/conf.d/$1_xdebug.ini
}

function deactivate_xdebug {
log_setting "remove /etc/php8/conf.d/$1_xdebug.ini | - zend_extension=xdebug"
if [ -f /etc/php8/conf.d/$1_xdebug.ini ] ; then
rm /etc/php8/conf.d/$1_xdebug.ini
fi
}
