...
log_setting "create /etc/php7/conf.d/$1_xdebug.ini | + zend_extension=xdebug"
echo zend_extension=xdebug > /etc/php7/conf.d/$1_xdebug.ini
}

function deactivate_xdebug {
log_setting "remove /etc/php7/conf.d/$1_xdebug.ini | - zend_extension=xdebug"
if [ -f /etc/php7/conf.d/$1_xdebug.ini ] ; then
rm /etc/php7/conf.d/$1_xdebug.ini
fi
}
